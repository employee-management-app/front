...
const MapMarker = ({ icon: Icon, color, showWarning, className, onClick }) => (
...
export const GoogleMap = ({ markers, selected, offset = DEFAULT_OFFSET, onSelect }) => {
  ...
  const { clusters, supercluster } = useSupercluster({
    points: markers.map(({ id, color, icon, showWarning, lat, lng }) => ({
      type: 'Feature',
      properties: { id, color, icon, showWarning },
      geometry: {
        type: 'Point',
        coordinates: [lng, lat],
      },
  ...
  return (
    <GoogleMapReact
      bootstrapURLKeys={{ key: process.env.REACT_APP_GOOGLE_MAPS_API_KEY, libraries: 'places', language: 'pl' }}
      defaultCenter={DEFAULT_CENTER}
      zoom={zoom}
      options={{ maxZoom: 18, zoomControl: false, mapId: process.env.REACT_APP_GOOGLE_MAPS_ID }}
    ...
    >
      {clusters.map((cluster) => {
        ...
        const { cluster: isCluster, point_count: pointCount, id, icon, color, showWarning } = cluster.properties;
