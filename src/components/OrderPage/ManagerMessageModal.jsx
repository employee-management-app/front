...
export const ManagerMessageModal = ({ orderId, message }) => {
  const dispatch = useDispatch();
  const { pushNotification } = useNotification();
  const { isVisible, hideModal } = useModalVisibility('ManagerMessageModal');
    ...
    >
      ...
      >
        <Grid space={SPACES.XL}>
          <GridEl size="12">
            <Field label="Description for managers">
              <Textarea
                value={fields.managerMessage}
                placeholder="Description for managers"
                size="medium"
